    ...
    <section id="history">
      <div class="frame">
        ...
        <div class="flex justify-around items-center mx-3 mb-3 font-bold">
          <div
            @click="setType('proses')"
            :class="
              type == 'proses'
                ? 'border-b-2 border-b-[#A00703] text-black'
                : 'border-b-2 border-b-[#fff] text-gray-600'
            "
            class="cursor-pointer pt-2 pb-2 w-full text-center hover:text-black hover:bg-gray-200"
          >
            Dalam Proses
          </div>
          <div
            ...
            :class="
              type == 'lunas'
                ...
                : 'border-b-2 border-b-[#fff] text-gray-600'
            "
            class="cursor-pointer pb-2 pt-2 w-full text-center hover:text-black hover:bg-gray-200"
          >
            Selesai
          </div>
        </div>
        ...
        <div
          v-if="type === 'proses'"
          v-for="data in dataPending"
          :key="data.nota"
          class="h-full border-2 border-[#00000020] mb-4 mx-3 rounded-sm"
        >
          <div class="flex flex-col">
            <div
              class="flex justify-between items-center px-3 text-black pt-4 pb-3"
            >
              <span>{{ data.orderType.name.toUpperCase() }}</span>
              <span>
                {{ this.formatDate(data.data.guest_detail.guest_addr.dateadd) }}
              </span>
            </div>
            <hr class="mx-3" />
            <div class="flex justify-between items-center px-3 text-black py-3">
              ...
              <span>{{
                data.data.restaurant_table ? data.data.restaurant_table : "-"
              }}</span>
            </div>
            <div class="flex justify-between items-center px-3 text-black py-3">
            ...
            <div class="flex justify-between items-center px-3 font-bold py-3">
              ...
              <span class="text-black">{{
                formatCurrency(data.data.payment.gtotal)
              }}</span>
            </div>
            <button
              @click="toDetail(data.nota)"
              class="border border-red-600 mx-3 py-2 mb-4 rounded-lg hover:bg-red-600 text-red-600 hover:text-white ease-linear duration-200"
            >
        ...
        <!-- sisi lunas -->
        <div
          v-else-if="type === 'lunas'"
          v-for="dataLunas in dataSuccess"
          :key="dataLunas.nota"
          class="h-full border-2 border-[#00000020] mb-4 mx-3 rounded-sm"
        >
          <div class="flex flex-col">
            ...
            >
              ...
              <span
                >{{
                  this.formatDate(
                    dataLunas.data.guest_detail.guest_addr.dateadd
                  )
                }}
              </span>
            </div>
            ...
            <div class="flex justify-between items-center px-3 font-bold py-3">
              <span class="text-gray-600">Grand Total</span>
              <span class="text-black">{{
                formatCurrency(dataLunas.data.payment.gtotal)
              }}</span>
            </div>
            <button
              @click="toDetail(dataLunas.nota)"
              class="border border-red-600 mx-3 py-2 mb-4 rounded-lg hover:bg-red-600 text-red-600 hover:text-white ease-linear duration-200"
            >
              <span>LIHAT DETAIL</span>
            </button>
        ...
        >
          <div class="flex flex-col">
            <div
              class="flex justify-center items-center px-3 text-black pt-4 pb-3"
            >
  ...
  mounted() {
    this.locId =
      localStorage.getItem("location") === null
        ? null
        : atob(localStorage.getItem("location"));
    const historyTemp =
    ...
    if (this.history) {
      this.dataPending = this.history
        ...
        .sort(
          (a, b) =>
            new Date(b.data.guest_detail.guest_addr.dateadd) -
            new Date(a.data.guest_detail.guest_addr.dateadd)
        );
      this.dataSuccess = this.history
    ...
    this.navbarTo = "/service/detail/" + location + urlTable;
  },
  methods: {
    setType(type) {
      this.type = type;
    ...
    toDetail(nota) {
      this.$router.push("/site/receipt/" + nota);
    },
